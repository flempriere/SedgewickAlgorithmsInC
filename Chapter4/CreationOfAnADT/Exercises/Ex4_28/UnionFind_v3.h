 ...
 * that performs a find and union in one call to the ADT and a function
 * that returns the number of nodes connected to a given node.
 *
 * @version 2.1
 * @date 2025-03-11
 * 
 ...
 * @param j second node to connect
 *
 * @return size_t index of canonical node if nodes are already connected else
 * @return NOT_FOUND.
 */
size_t UFunionFind(size_t i, size_t j);

/**
 * @brief Returns the number of nodes connected to a node i.
 *
 * All nodes are considered connected to themselves.
 * 
 * @param i 
 * @return size_t >= 1.
 */
