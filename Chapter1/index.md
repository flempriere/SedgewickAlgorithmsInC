...
- At most $`\lg\left(N\right) + 1`$ checks `i != id[i]` since the distance from any node to the root is at most $`\lg\left(N\right)`$ and we check every node on the path.
- At most $`\lg\left(N\right)`$ updates `i = id[i]` for the same argument above.

This leads to an upper bound of:

```math
\begin{align}
2c\left(\lg\left(N\right) + 1\right)
\end{align}
```
...
```math
\begin{align}
4Mc\left(\lg\left(N\right) + 1\right)
\end{align}
```
...
the same component). This means that the following union can happen at most $`\text{min}(M, N - 1)`$ times.
...
```math
\begin{align}
c\left(3\text{min}\left(M, N-1\right) + M\left(4\lg\left(N\right) + 5\right)\right)
\end{align}
```
...
```math
\begin{align}
4Mc\left(2 + \lg\left(N\right)\right)
\end{align}
```
...
- at least $`10`$ instructions per while loop iteration
- at least $`N`$ instructions per for loop iteration
- performs at least $`M`$ iterations over the while loop.

```math
\begin{align}
10 \times 10^9 \times 10^6 &= 10^{16} \text{instructions} \\
&= 10^7 \text{seconds}
\end{align}
```

Which is about $`115`$ days
...
Tree is just the root. $`\bar{d} = 0`$ Maximum depth is $`0`$

$`n = 1`$:

Tree is one root, and one node at depth $`1`$.

```math
\bar{d} = \frac{1}{2}\left(0 + 1\right) = \frac{1}{2}
```
...
$`n = 2`$:
 ...
 Tree is one root, two nodes at depth $`1`$, and one node at depth $`2\bar{d} = \frac{1}{4}\left(0 + 2 \times 1 + 2\right) = 1`$ Maximum depth is $`2`$
...
For fun lets check, $`n = 3`$, by formula $`\bar{d} = \frac{3}{2}`$ By first principles

```math
\begin{align}
 \bar{d} &= \frac{1}{8}\left(0 + 3 \times 1 + 3 \times 2 + 3\right) \\
        &= \frac{12}{8}\\
        &= \frac{3}{2}
\end{align}
```
...
*The weighted quick-union algorithm follows at most $`2\lg N`$ pointers to determine whether two of $`N`$ objects are connected.*
...
**Lemma 2**
A tree built by Exercise 1.20 is always of height $`h \leq \lg N`$ where $`N`$ is the number of nodes in the tree.

**Proof**

*Base case:* a single node forms a tree of height $`0.`$ Since $`0 \leq \lg(1) = 0`$ this is trivially true.
...
```math
\begin{align}
    P(N, M) &= \frac{C\left(N, M\right)}{G(N, M)}
\end{align}
```
